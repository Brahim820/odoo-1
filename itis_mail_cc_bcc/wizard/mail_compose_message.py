...
class MailComposer(models.TransientModel):
    ...
    email_cc = fields.Many2many('res.partner', 'mailcompose_partnercc_rel',
                                'mailid', 'partner_id', string="Cc", help='Carbon copy message recipients')
    email_bcc = fields.Many2many('res.partner', 'mailcompose_partnerbcc_rel', 'mailid',
                                 'partner_id', string="Bcc", help='Blind Carbon copy message recipients')

    @api.model
    def default_get(self, fields):
        result = super(MailComposer, self).default_get(fields)
        if result.get('model', False):
            follower_ids = self.env['document.followers'].search(
                [('model_id.model', '=', result.get('model')), ('auto_add_followers', '=', True)])
            result['email_cc'] = [(4, user.partner_id.id)
                                  for user in follower_ids.internal_follower_ids]
            result['email_bcc'] = [(4, user.partner_id.id)
                                   for user in follower_ids.external_follower_ids]
        return result
    ...
    def get_mail_values(self, res_ids):
        ...
        for res_id in res_ids:
            ...
            if mass_mail_mode and self.model:
                ...
                mail_values.update(notification=True, model=self.model,
                                   res_id=res_id, record_name=False)
                ...
                mail_values['recipient_ids'] = [(4, id)
                                                for id in mail_values.pop('partner_ids', [])]

                # process attachments: should not be encoded before being processed by message_post / mail_mail create
                mail_values['attachments'] = [(name, base64.b64decode(enc_cont))
