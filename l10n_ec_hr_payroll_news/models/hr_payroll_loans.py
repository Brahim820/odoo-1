...
class HrPayslipLoans(models.Model):
    ...
    name = fields.Char(_('Concept'), rquired=True, states={
    ...
    amount = fields.Float(_('Amount'), required=True, states={
                          'draft': [('readonly', False)]})
    ...
    pay_from = fields.Char(compute=_get_pay_from,
                           string=_('Pay From'), store=True)
    ...
    payment_id = fields.Many2one('account.payment', _(
        'Payment'), compute=_compute_payment)
