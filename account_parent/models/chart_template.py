    ...
    def update_template_property_field(self, account_id, company):
        ...
        if value:
            ...
            properties = PropertyObj.search(
                [('res_id', '=', self.id), ('name', '=', 'property_temp_related_account_id'), ('company_id', '=', company.id)])
            if properties:
    ...
    def generate_account(self, tax_template_ref, acc_template_ref, code_digits, company):
        ...
        for account_template_obj in account_template_objs:
            account_template_obj.update_template_property_field(
                account_template_account_dict[account_template_obj.id], company)
    ...
    def update_generated_account(self, tax_template_ref=[], code_digits=1, company=False, importing_parent=False):
        ...
        if company.bank_account_code_prefix:
            if code_account_dict.get(company.bank_account_code_prefix, False):
                ...
                parent_account_id = account_obj.with_context({'show_parent_account': True}).search(
                    [('code', '=', company.bank_account_code_prefix), ('company_id', '=', company.id)], limit=1)
        ...
        if company.cash_account_code_prefix:
            if code_account_dict.get(company.cash_account_code_prefix, False):
                ...
                parent_account_id = account_obj.with_context({'show_parent_account': True}).search(
                    [('code', '=', company.cash_account_code_prefix), ('company_id', '=', company.id)], limit=1)
...
#         account and account.with_context({'company_id':company.id}).write({'parent_id':parent_account_id.id})
        ...
        for account_template in all_acc_templates:
            ...
            account_template.property_temp_related_account_id.write(
                {'parent_id': account_template.parent_id.property_temp_related_account_id.id})
        return True
