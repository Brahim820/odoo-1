...
class HrPayslipEmployees(models.TransientModel):
    ...
    def onchange_slip_id(self):
        ...
        for emp in employees:
            ...
            if self.run_id.payroll_type.name == "Quincenal":
                date_in = dt.strptime(emp.mapped("contract_id").date_start, "%Y-%m-%d")
                if date_in.month == date_run.month and date_in.year == date_run.year:
                    if int(date_in.day) >= int(11):
                        no_ids.append(emp.id)

            if emp.mapped("contract_id").date_end and (
                emp.mapped("contract_id").date_end < self.run_id.date_end
            ):
                no_ids.append(emp.id)
            if (
                emp.mapped("contract_id").state == "close"
                and not emp.mapped("contract_id").date_end
            ):
                no_ids.append(emp.id)
                continue
    ...
    def compute_sheet(self):
        ...
        for employee in employees:
            ...
            contract = {"id": False, "type_id": False}
            if slip_data["value"].get("contract_id"):
