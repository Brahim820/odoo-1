...
class WizardHrPayslipNews(models.TransientModel):
    ...
    def generate_news(self):
        ...
        for row in self:
            ...
            date_from = "{}-{}-{}".format(
                row.generate_date[0:4], row.generate_date[5:7], "01"
            )
            ...
            elif row.generate_option == "employees" and row.generate_employee_ids:
            ...
            for contr in contract_obj.browse(contract_ids):
                amount = 0
                wage = payroll_obj.get_contract_info(contr.id, date_from, date_to)[
                    "wage"
                ]
                vals["employee_id"] = contr.employee_id.id
                ...
                if row.generate_type == "net_percent":
                    ...
                    self.env.cr.execute(
                        query.format(
                            date_from,
                            date_to,
                            tuple(["ALW", "INGRESONOGRAVADO", "INGNOGRAV", "INGGRAV"]),
                        )
                    )
                    in_data = self.env.cr.dictfetchall()[0]
                    incomming = sum([wage, in_data.get("amount", False)])
                    self.env.cr.execute(
                        query.format(
                            date_from, date_to, tuple(["DED", "LOAN", "SUBIESS"])
                        )
                    )
                    out_data = self.env.cr.dictfetchall()[0]
                    outcomming = out_data.get("amount", False)
