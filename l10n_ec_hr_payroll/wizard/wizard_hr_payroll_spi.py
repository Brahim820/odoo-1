...
from openerp.addons.l10n_ec_payment.models.es_num2word import to_word
...
class WizradHrPayslipSPI(models.TransientModel):
    ...
    def _onchange_bank_account(self):
        if self.bank_account_id:
            ...
            payment_methods = self.bank_account_id.bank_id.journal_id.outbound_payment_method_ids
            self.payment_method_id = payment_methods and payment_methods[0] or False
            # Set payment method domain (restrict to methods enabled for the journal and to selected payment type)
            return {'domain': {'payment_method_id': [('payment_type', '=', 'outbound'), ('id', 'in', payment_methods.ids)],
                               'payment_method_check_id': [('payment_type', '=', 'outbound'), ('id', 'in', payment_methods.ids)]}}
        return {}
    ...
    def _compute_payment_data(self, slip_id):
        ...
        for line in slip_id.move_id.line_ids:
            if line.account_id.internal_type == 'payable' and line.partner_id.id == slip_id.employee_id.address_home_id.id:
    ...
    def get_spi(self):
        ...
        for row in self:
            if not row.bank_account_id.bank_id.journal_id:
                raise UserError(_('Error'), _('The bank {} has not set up a diary for payroll payments!'.format(
            ...
            date = datetime.strptime(self.payment_date,
                                     '%Y-%m-%d').strftime('%d%m%y')
            file_name = '{}_{}.txt'.format('SPI', date)
            slip_ids = row.slip_id.mapped('slip_ids').filtered(
                lambda x: x.exclude is False)
            amount = '{:.2f}'.format(
                row.slip_id.transfer_amount).replace('.', '')
            file_data.write(
                template.format('D',
                                row.slip_id.company_id.name.upper(),
                                row.bank_account_id.acc_number, amount, date))
            if not slip_ids:
                raise UserError(
                    _('Error'), _('No slip generates for this month'))
            for slip in slip_ids:
                payment_data = self._compute_payment_data(slip)
                if slip.employee_id.bank_account_id:
                    amount = '{:.2f}'.format(
                        self._compute_pay_amount(slip)).replace('.', '')
                    name = slip.employee_id.name
                    file_data.write(
                        template.format('C',
                                        self.clean_not_unicode(name.upper()), slip.employee_id.
                                        bank_account_id.acc_number, amount,
                                        date))
                    payment = {
                        'model': 'hr.payslip',
                ...
                if not slip.payment_ids:
                    ...
                    for line in slip.move_id.line_ids:
                        if line.account_id.internal_type == 'payable' and line.partner_id.id == slip.employee_id.address_home_id.id:
                            account_move_lines_to_reconcile |= line
