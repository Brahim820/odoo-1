...
class HrEmployee(models.Model):
    ...
    def split_name(self, name):
        vals = {"names": "", "lastname": "", "second_lastname": ""}
    ...
    def update_employee_names(self):
        employees = self.search([("names", "=", False), ("lastname", "=", False)])
        if employees:
            for ee in employees:
        ...
        for row in self:
            if row.family_ids:
                row.children = family_obj.search_count(
                    [("employee_id", "=", row.id), ("relation", "=", "child")]
                )

    names = fields.Char(_("Name's"))
    ...
    old_age = fields.Boolean(
        compute=_check_age, string=("Old Age"), store=False, oldname="tercera_edad"
    )
        ...
        for row in self:
            if row.identification_id is False and row.passport_id is False:
                raise ValidationError(
                    _(
                        "The employee must have an identification number or passport number"
                    )
                )
            ...
            if identification_ids and row.id not in identification_ids:
                raise ValidationError(
                    _("The identification number must be unique per employee")
                )
            passport_ids = [
                i.id
                for i in self.search(
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        ...
        if context.get("show_unemployed", False):
