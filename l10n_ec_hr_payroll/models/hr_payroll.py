...
class HrPayslipRun(models.Model):
    ...
    transfer_amount = fields.Float(
        "Amount to pay by transfer", compute=_get_slip_pay_amount
    )
    check_amount = fields.Float("Amount to pay by check", compute=_get_slip_pay_amount)
    exclude_amount = fields.Float("Exclude amount", compute=_get_slip_pay_amount)
    ...
    def _onchange_date_start(self):
        ...
        date_end = date_start + relativedelta.relativedelta(months=+1, day=1, days=-1)
...
class HrPayslip(models.Model):
    ...
    def get_contract_info(self, cr, uid, contract_id, date_from, date_to, context=None):
        ...
        hjob_obj = self.pool.get("hr.contract.job")
        hwage_obj = self.pool.get("hr.contract.wage")
        hjob_ids = hjob_obj.search(cr, uid, domain)
        hwage_ids = hwage_obj.search(cr, uid, domain)
        if hjob_ids:
            for hj in hjob_obj.browse(cr, uid, hjob_ids):
                if hj.date >= date_to:
                    res["job_id"] = hj.old_job_id.id
        if hwage_ids:
            for hw in hwage_obj.browse(cr, uid, hwage_ids):
                if hw.date >= date_to:
    ...
    def get_payslip_lines(self, cr, uid, contract_ids, payslip_id, context):
        ...
        class Sri(BrowsableObject):
            def tax_rent(
                ...
                table_ids = table_obj.search(
                    self.cr, self.uid, [("year", "=", year), ("active", "=", True)]
                )
                if table_ids:
        ...
        class Utils(BrowsableObject):
                ...
                if method == "payslip":
                    ...
                    if line_id:
                        amount = sum(
                            [
                                prov.amount
                                for prov in line_obj.browse(self.cr, self.uid, line_id)
                            ]
                        )
                ...
                if payslip_id and rule_id:
                    ...
                    if line_id:
                        amount = sum(
                            [
                                ded.amount
                                for ded in line_obj.browse(self.cr, self.uid, line_id)
                            ]
                        )
                if amount > 0:
                    return amount
            ...
            def invoices(self, employee, date_from, date_to):
                ...
                journal_id = journal_obj.search(
                    self.cr, self.uid, [("payroll_discount", "=", True)]
                )
                ...
                if payment_id:
                    amount = sum(
                        [
                            pay.amount
                            for pay in payment_obj.browse(self.cr, self.uid, payment_id)
                        ]
                    )
                if amount > 0:
                    return amount
        ...
        utils_obj = Utils(self.pool, cr, uid, payslip.employee_id.id, payslip_obj)
        baselocaldict = {
        ...
        for contract in self.pool.get("hr.contract").browse(
            ...
            for rule_id in sorted_rule_ids:
                rule = obj_rule.browse(cr, uid, rule_id, context=context)
            ...
            for rule in obj_rule.browse(cr, uid, payroll_rule_ids, context=context):
                ...
                if (
                    ...
                    blacklist += [
                        id
                        for id, seq in self.pool.get(
                            "hr.salary.rule"
                        )._recursive_search_of_rules(cr, uid, [rule], context=context)
                    ]
    ...
    def compute_worked_days(self):
        for payslip in self:
            ...
            worked_days = [
                (0, 0, worked_days)
                for worked_days in self.get_worked_day_lines(
                    contract_ids, payslip.date_from, payslip.date_to
                )
            ]
            ...
            if payroll_type.overtime:
                ...
                for over in overtime_ids:
                    ...
                    if over.overtime_100:
                        ...
                        res += [inputs]
                if payroll_type.invoices and journal_ids:
                    ...
                    for inv in payment_ids.mapped("invoice_ids"):
                        inputs = {
                            "name": inv.name_get()[0][1],
                            "code": "DEDF",
                            "contract_id": contract.id,
                            "invoice_id": inv.id,
                            "quantity": 1,
