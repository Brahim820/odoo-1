...
class HrSriRetention(models.Model):
    ...
    name = fields.Char(string=_('Description'),
                       required=True, default=_default_name)
    year = fields.Char(string=_('Year'), default=_default_year, required=True)
    active = fields.Boolean(string=_('Active'), default=True)
    line_ids = fields.One2many(
        'hr.sri.retention.line', 'ret_id', string=_('Details'))
    max_deductible = fields.Float(string=_('Maximum deductible'), default=0.0)
    projection_ids = fields.One2many(
        'hr.sri.projection', 'ret_id', string=_('Projections'))
    old_age = fields.Float(
        string=_('Exoneration for the elderly'), required=True)
...
class HrSriRetentionLine(models.Model):
    ...
    @api.multi
    @api.depends('basic_fraction', 'excess_up', 'basic_fraction_tax')
    def _compute_all(self):
        for row in self:
            row.monthly_basic_fraction = row.basic_fraction / 12.0
            row.monthly_excess_up = row.excess_up / 12.0
            row.monthly_basic_fraction_tax = row.basic_fraction_tax / 12.0

    basic_fraction = fields.Float(
        string=_('Basic Fraction'), required=True, default=0.0)
    monthly_basic_fraction = fields.Float(
        string=_('Monthly Basic Fraction'), compute=_compute_all, store=True)
    excess_up = fields.Float(string=_('Excess Up'), required=True, default=0.0)
    monthly_excess_up = fields.Float(
        string=_('Monthly Excess Up'), compute=_compute_all, store=True)
    basic_fraction_tax = fields.Integer(
        string=_('Basic Fraction Tax'), required=True, default=0)
    monthly_basic_fraction_tax = fields.Float(
        string=_('Monthly Basic Fraction Tax'), compute=_compute_all, store=True)
    percent = fields.Integer(
        string=_('Excess Fraction (%)'), required=True, default=0)
...
class HrSriProjection(models.Model):
    ...
    deduction_id = fields.Many2one(
        'hr.sri.deduction', string=_('Deduction'), required=True)
    amount = fields.Float(string=_('Maximum Deductible'), required=True)
    ret_id = fields.Many2one('hr.sri.retention', string=_(
        'Retention Table'), ondelete='cascade')
...
class HrSriAnnualProjection(models.Model):
    ...
    monthly_amount = fields.Float(
        compute=_compute_all, string=_('Monthly Amount'), store=True)
    amount = fields.Float(compute=_compute_all, string=_('Amount'), store=True)
...
class HrSriAnnualProjectionLine(models.Model):
    ...
    projection_id = fields.Many2one(
        'hr.sri.annual.projection', string=_('Year'))
...
class HrSriAnnualRentTax(models.Model):
    ...
    line_ids = fields.One2many(
        'hr.sri.rent.tax', 'rent_id', string=_('Tax Rent'))
    contract_id = fields.Many2one('hr.contract', string=_('Contract'))
...
class HrSriRentTax(models.Model):
    ...
    rent_id = fields.Many2one('hr.sri.annual.rent.tax',
                              string=_('Annual Rent Tax'))
