...
class payslip_report(report_sxw.rml_parse):
    ...
    def get_period_name(self, obj):
        res = None
        ds = datetime.strptime(obj.date_to, '%Y-%m-%d')
    ...
    def get_details(self, obj, hr_payslip_id=None):
        ...
        if ids:
            self.cr.execute("""SELECT pl.id, pl.category_id FROM hr_payslip_line as pl \
				...
				WHERE pl.id in %s and rc.code in ('BASIC','ALW','INGRESONOGRAVADO', 'INGNOGRAV', 'INGGRAV', 'HE050', 'HE100') \
				GROUP BY rc.parent_id, sr.sequence, pl.id, pl.category_id \
				ORDER BY sr.sequence, rc.parent_id""", (tuple(ids),))
            for x in self.cr.fetchall():
                result.setdefault(x[1], [])
                result[x[1]].append(x[0])
            for key, value in result.iteritems():
