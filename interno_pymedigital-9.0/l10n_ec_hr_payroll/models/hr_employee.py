...
class HrEmployee(models.Model):
        ...
        for row in self:
            if row.birthday:
                ...
                if date_birthday <= datetime.today():
                    ...
                    date_birth = date(int(birth[0]), int(
                        birth[1]), int(birth[2]))
    ...
    def onchange_name(self):
        name = ''
        for row in self:
            name = '%s %s %s' % (row.lastname or '',
                                 row.second_lastname or '', row.names or '')
            row.name = name
    ...
    family_ids = fields.One2many(
        'hr.family', 'employee_id', string=_('Family'))
    children = fields.Integer(compute=_get_children,
                              string="Children", store=True)
    marital = fields.Selection(
        selection_add=[('union_fact', _('Union of Fact'))])
