...
class HrContractFinish(models.TransientModel):
    ...
    contract_id = fields.Many2one(
        'hr.contract', string='Contract', default=lambda self: self._context.get('active_id'))
    reason_id = fields.Many2one(
        'hr.contract.finish.reason', string='Finish Reason')
    ...
    def close(self):
        if self.contract_id.employee_id.active:
            self.contract_id.employee_id.toggle_active()
