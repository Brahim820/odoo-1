...
class AccountInvoice(models.Model):
    ...
    def prepare_detallecompras_dict(self):
        ...
        for inv in self:
            ...
            if inv.fechaemiret1:
                fechaEmiRet1 = datetime.strptime(inv.fechaemiret1, "%Y-%m-%d").strftime(
                    "%d/%m/%Y"
                )
            else:
                fechaEmiRet1 = ""
                ...
                pagoExterior.append(
                    OrderedDict(
                        [
                            ...
                            (
                                "aplicConvDobTrib",
                                fiscal.doble_tributacion and "SI" or "NO",
                            ),
            ...
            for line in inv.sri_ats_line_ids:
                ...
                if fiscal.identificacion_id.code == "P":
                    vals.update(OrderedDict([("tipoProv", inv.persona_id.tipoprov)]))
    ...
    @api.multi
    @api.onchange('establecimiento', 'puntoemision', 'secuencial')
    ...
    def button_anular_secuencial(self):
        for inv in self:
            ...
            if inv.type in ('in_invoice','in_refund'):
                ...
                autorizacion = inv.r_autorizacion_id
