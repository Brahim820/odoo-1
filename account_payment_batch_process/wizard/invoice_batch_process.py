...
class InvoiceCustomerPaymentLine(models.TransientModel):
    ...
    def _onchange_amount(self):
        self.check_amount_in_words = to_word(self.receiving_amt)
...
class InvoicePaymentLine(models.TransientModel):
    ...
    def _onchange_amount(self):
        self.check_amount_in_words = to_word(self.paying_amt)
...
class AccountRegisterPayments(models.TransientModel):
    ...
    def make_payments(self):
        ...
        if self.is_customer:
            ...
            if float_compare(self.total_customer_pay_amount, self.cheque_amount, precision_digits=2) != 0:
            ...
            for paym in self.invoice_customer_payments:
                if paym.receiving_amt > 0:
                    ...
                    if partner_id in data:
                        ...
                        try:
                            amount_word = to_word(amount_total)
                        except:
                            amount_word = ''
            ...
            if float_compare(self.total_pay_amount, self.cheque_amount, precision_digits=2) != 0:
                raise ValidationError(_('Verification Failed! Total Invoices'
                                        ' Amount and Check amount does not'
                                        ' match!.'))
                ...
                if paym.paying_amt > 0:
                    ...
                    if partner_id in data:
                        ...
                        if self.communication:
                            ...
                            memo = str(paym.invoice_id.number)
                        # Calculate amount in words
                        amount_word = to_word(paym.paying_amt)
                        data.update({
                            partner_id:
                                {'partner_id': partner_id,
                                 'partner_type': INV_TO_PARTN[paym.invoice_id.type],  # noqa
