...
class AccountPayment(models.Model):
    ...
    offline = fields.Boolean('Offline post Payment')
        ...
        if 'group_data' in self._context:
            ...
            for inv in self._context.get('group_data')[p_id]['inv_val']:
                ...
                if 'is_customer' in self._context and self._context.get('is_customer'):
                    amt = -(self._context.get('group_data')[p_id]['inv_val'][inv]['receiving_amt'])
                else:
                ...
                debit, credit, amount_currency, currency_id = aml_obj.with_context(date=self.payment_date).compute_amount_fields(
                    amt, self.currency_id, self.company_id.currency_id, invoice_currency)
                ...
                counterpart_aml_dict.update(self._get_counterpart_move_line_vals(current_invoice))
                ...
                if 'is_customer' in self._context and self._context.get('is_customer'):
                    ...
                    if handling == 'reconcile' and payment_difference:
                        ...
                        debit_wo, credit_wo, amount_currency_wo, currency_id = aml_obj.with_context(date=self.payment_date).compute_amount_fields(
                            payment_difference, self.currency_id, self.company_id.currency_id, invoice_currency)
    ...
    @api.v7
    def offline_post_payment(self, cr, uid):
        paymen_data = self.search(
            cr, uid, [('offline', '=', True), ('state', '=', 'draft')], limit=1)
        for pay in self.browse(cr, uid, paymen_data):
            logging.info('Starting payment registration to the customer {} for the amount of {}'.format(
                pay.partner_id.name, pay.amount))
            pay.post()
            logging.info('Payment registration to the customer {} for the amount of {} finished'.format(
                pay.partner_id.name, pay.amount))
        return True
